...
def weighted_sample(sample_constant, sku_count):
...
audit_df = df.merge(sample_df, on='Grainger_SKU', how='inner', suffixes=('_',''))
audit_df.drop('L3_', axis=1, inplace=True)
print('audit_df size: {}'.format(audit_df.shape))
fd.sample_data_out(settings.directory_name, sku_count, audit_df, df, sample)