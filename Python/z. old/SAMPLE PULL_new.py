...
def percentage_sample(sku_count):
...
def weighted_sample(report_type, sku_count):
    ...
    if report_type == 'node':
        ...
        for index, row in sku_count.iterrows():
            samp_size = 0.10
            if row['Supplier Name']<100:
                samp_size = samp_size
            elif (row['Supplier Name']>=100) & (row['SKU Count']<200):
                samp_size = ceil((samp_size*0.5)*100)/100
            elif (row['Supplier Name']>200) & (row['SKU Count']<500):
                samp_size = ceil((samp_size*0.3)*100)/100
            elif row['Supplier Name']>=500:
                samp_size = round(samp_size*0.2, 2)    
            count = (int(row['Supplier Name']*samp_size))
            #Deal with small sample sizes -- if sample percentage as calculated above is <1, choose 2 SKUs from the sample if possible, or 1 if there is only one present
            if count <= 1:
                if row['Supplier Name'] >= 2:
                    count = 2
                else:
                    count = 1
                sample[row['Category ID']] = count
...
def create_sample(sample, df, skus):
...
if sample_type in ['1', '%', '5', 'percent', 'percentage']:
    sample = percentage_sample(sku_count)
elif sample_type in ['2', 'weighted', 'weight']:
    report_type = report_type()
    sample = weighted_sample(report_type, sku_count)
...
sample_df = create_sample(sample, df, skus)  #sample can be either a percent entered by the user OR a dictionary of weighted counts by node size
...
fd.sample_data_out(settings.directory_name, sample_df, atts, skus, sku_count, df)