...
class PostgresDatabase:
    ...
    def __init__(self, db="gt-web-stage"):
    ...
    def _connect2postgres_(self, db):
        ...
        host = "gt-st-database.eastus.cloudapp.azure.com"
        engine = create_engine(
            "postgresql+psycopg2://{username}:{password}@{host}/{db}".format(
                username=args[0], password=args[1], host=host, db=db
            )
        )
        conn = engine.connect()
        return conn
    ...
    def _getcreds_():
        ...
        username = parser.get("POSTGRES", "USER")
        password = parser.get("POSTGRES", "PASS")
